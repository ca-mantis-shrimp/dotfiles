[{1 :mfussenegger/nvim-dap
  ...
  :config (fn []
            (let [dap (require :dap)
              ...
              (dap.listeners.after.event_initialized.dapui_config (fn []
                                                                    (dapui.open)))
              (dap.listeners.before.event_terminated.dapui_config (fn []
                                                                    (dapui.close)))
              (dap.listeners.before.event_exited.dapui_config (fn []
                                                                (dapui.close)))
              ...
              (let [map _G.vim.keymap.set]
                ...
                (map :n :<leader>dB
                     (fn []
                       (dap.set_breakpoint (_G.vim.fn.input "Breakpoint condition: ")))

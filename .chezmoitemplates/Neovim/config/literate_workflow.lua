-- [nfnl] config/literate_workflow.fnl
local M = {}
...
M["format-lua"] = function(filepath)
	...
	if not ok then
		vim.notify("conform not available, skipping format", vim.log.levels.WARN)
		return true
	else
		local lua_bufnr = vim.fn.bufadd(filepath)
		vim.fn.bufload(lua_bufnr)
		conform.format({ bufnr = lua_bufnr, async = false })
		local function _10_()
			return vim.cmd.write()
		end
		vim.api.nvim_buf_call(lua_bufnr, _10_)
		vim.api.nvim_buf_delete(lua_bufnr, { force = true, unload = true })
		return true
	end
end
...
vim.api.nvim_create_autocmd("FileType", { pattern = "norg", callback = _20_ })
return M

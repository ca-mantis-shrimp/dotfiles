...
(_G.vim.api.nvim_create_autocmd "LspAttach"
            {:group (_G.vim.api.nvim_create_augroup "kickstart-lsp-attach" {:clear true}) :callback (fn [event]
                    (local map (fn [keys func desc]
                     (_G.vim.keymap.set "n" keys func {:buffer event.buf :desc (.. "LSP: " desc)})))
                    ...
                    (when (and client client.server_capabilities.documentHighlightProvider)
                      (_G.vim.api.nvim_create_autocmd ["CursorHold" "CursorHoldI"]
                              ...
                              :callback _G.vim.lsp.buf.clear_references}))
                                ...
                                (when (and client client.server_capabilities.inlayHintProvider _G.vim.lsp.inlay_hint)
                                  (map "<leader>th"
                                       (fn []
                                         (_G.vim.lsp.inlay_hint.enable 0 (not (_G.vim.lsp.inlay_hint.is_enabled))))

...
;; Chezmoi
(_G.vim.keymap.set :n :<leader><tab>c "<cmd>tcd ~/.local/share/chezmoi<CR>" {:desc "CD to chezmoi in current tab"})
...
(_G.vim.keymap.set :n :<leader><tab>a #(os.execute "chezmoi apply") {:desc "Apply to Chezmoi"})
...
(_G.vim.keymap.set :n :<leader>a #(: (harpoon:list) :add) {:desc "Add current file to harpoon list"})
(_G.vim.keymap.set :n :<leader>H #(harpoon.ui:toggle_quick_menu (harpoon:list)) {:desc "Toggle Harpoon Quick Menu"})

(_G.vim.keymap.set :n :<C-1> #(: (harpoon:list) :select 1) {:desc "Select Harpoon item 1"})
(_G.vim.keymap.set :n :<C-2> #(: (harpoon:list) :select 2) {:desc "Select Harpoon item 2"})
(_G.vim.keymap.set :n :<C-3> #(: (harpoon:list) :select 3) {:desc "Select Harpoon item 3"})
(_G.vim.keymap.set :n :<C-4> #(: (harpoon:list) :select 4) {:desc "Select Harpoon item 4"})
(_G.vim.keymap.set :n :<C-5> #(: (harpoon:list) :select 5) {:desc "Select Harpoon item 5"})
(_G.vim.keymap.set :n :<C-6> #(: (harpoon:list) :select 6) {:desc "Select Harpoon item 6"})
...
(_G.vim.keymap.set :n :<leader>sb
  (fn []
    (let [query (_G.vim.fn.input "Search Google: ")]
      (when (not= query "")
        (_G.vim.cmd {:cmd "OpenBrowserSmartSearch" :args [(.. "'" query "'")]}))))

...
M["chezmoi-source-to-dest"] = function(source_path)
	...
	local patterns = {
		...
		{ "^.*/AppData/Local/remove_(.+)%.tmpl$", (home .. "/AppData/Local/%1") },
		{ "^.*/AppData/Local/remove_(.+)$", (home .. "/AppData/Local/%1") },
...
M["trace-pipeline"] = function(norg_file)
	...
	if not tangle_directive then
		...
		do
			local dest = M["chezmoi-source-to-dest"](tangle_path)
			if dest then
...
vim.api.nvim_create_autocmd("FileType", { pattern = "norg", callback = _13_ })
return M
